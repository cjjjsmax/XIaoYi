...
fun MyPostsScreen(
    ...
    fun ProductCardWithActions(
        ...
        val productData = product["product"] as? Map<*, *>
        val productName = productData?.get("title") as? String ?: ""
        val price = productData?.get("price")?.toString() ?: "0"
        val location = productData?.get("location") as? String ?: ""
        val imageUrl = productData?.get("images") as? String ?: ""
        Column {
            ProductCard(
            ...
            LazyVerticalGrid(
                ...
                items(products.size){ index ->
                    ...
                    ProductCardWithActions(
                        ...
                        onEdit = {
                            val productData = product["product"] as? Map<*, *>
                            val productId = (productData?.get("id") as? Number)?.toLong() ?: 0L
                            navController.navigate(Screen.EditProduct.route.replace("{productId}", productId.toString()))
                        },
                        onDelete = {
                            val productData = product["product"] as? Map<*, *>
                            selectedProductId = (productData?.get("id") as? Number)?.toLong() ?: 0L
